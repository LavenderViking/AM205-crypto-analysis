...
def jacobi(a,tol = 1.0e-9): # Jacobi method
    ...
    def rotatePair(a,p,i,j,k,l,s,tau):
        g,h = a[i,j],a[k,l]
	...
	# Rotation point and it's projection points on the diagonal:
        temp = a[k,l]
        (a[k,l],a[k,k],a[l,l]) = (0.0,a[k,k]-t*temp,a[l,l]+t*temp)

	# Rotation of elements:
        # Combines 3 cases of rotations:
        # i)   i < k
        # ii)  k < i < l
        # iii) l < i
        for i in range(n):
            # Values on the diagonal have already been changed above:
            if i==k or i==l: continue
            
            # Store indexes we will use for the rotation:
            lo,hi = (i,k) if i<=k else (k,i)
            le,ri = (i,l) if i<=l else (l,i)
            
            # Store temp value used for the rotation:
            temp = a[lo,hi]

            # Rotate the pair:
            rotatePair(a,p,lo,hi,le,ri,s,tau)

        # Update the eigenvector matrix:
        for i in range(n):
            temp = p[i,k]
            p[i,k] -= s*(p[i,l] + tau*p[i,k])
